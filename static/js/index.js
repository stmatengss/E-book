...
$(function(){
	...
	$('.input-panel.input-right .send').on('click', function(e){
        ...
        $element.removeClass('selected');
        $elementAll = $element.parent().parent();
        $newElementAll = $elementAll.clone(true);
        $elementAll.remove();
        $newElementAll.find('.comment-item').text($newElementAll.find('.comment-item').text()+'//'+Trim($('.input-panel.input-right .form-control').val()));
        $('.comment').prepend($newElementAll);
//		var $element = $('.comment .item.selected');
//		$element.removeClass('selected');
//		$newElement = $element.clone(true);

//		var num = $element.find('div:first').text();
//
//		$element.remove();
//         '<div style="float:left; width:28px; height: 14px; "><sub style="font-size:14px;">${num}</sub></div>' +
//                    		'<div style="float:left; width:408px;"><p class="comment-item item" data-lid="${lid}" style="text-indent:${dist}"><a>${text}</a></p></div>';
//		var content = '<div style="float:left; width:28px; height: 14px; "><sub style="font-size:14px;"><sub style="font-size:14px;">'
//		            +num+'</sup></div>' + Trim($('.input-panel.input-right .form-control').val())+'//';
...
//		//set last element
		...
		updateComment($newElementAll.find('.comment-item').attr('data-lid'), encodeURIComponent($newElementAll.prop("outerHTML")));
		//loadComments();
		//location.reload();
	});
...
//				$(($com).find("sup")[0]).text("");
//				$('.comment').append("<div style=\"float:left; width:28px; height: 12px; \"><sub style=\"font-size:12px;\">"+sup.toString()+"</sub></div>");
...
function addComment(num){
	...
	if (false) {
    	...
    	max += 1;
    	var templates = '<div class="comment-all"><div style="float:left; width:28px; height: 12px; "><p class="sup-p" style="font-size:12px;">${num}</p></div>' +
            		'<div style="float:left; width:408px;"><p class="comment-item item" data-lid="${lid}" style="text-indent:${dist}"><a>${text}</a></p></div></div>';
//		var templates =
//    		'<p class="comment-item item" data-lid="${lid}" style="text-indent:${dist}"><a><sup>${num}</sup>${text}</a></p>';
    	...
    	var images =
    	    '<div class="comment-all"><div style="float:left; width:28px; height: 12px; "><p class="sup-p" style="font-size:12px;">${num}</p></div>\
    		 <div class="comment-item" data-lid="${lid}"><div style="display:table;margin-left:${left}px">\
    			<div style="background: url(${src}) center center no-repeat;background-size: ${width}px ${height}px;width:${width}px;height:${height}px" class="img noise"></div>\
    		</div></div></div>';
